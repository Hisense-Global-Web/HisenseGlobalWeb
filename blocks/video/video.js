export default function decorate(block) {
  ...
  [...block.children].forEach((row) => {
    ...
    row.className = key;
    if (row.className === 'video-origin') {
      if (row.children[1].textContent.trim().toLowerCase() === 'internal') {
        row.setAttribute('data-video-origin', 'internal');
      } else {
        row.setAttribute('data-video-origin', 'vimeo');
      }
    }
