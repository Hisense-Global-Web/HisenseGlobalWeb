export default function decorate(block) {
  ...
  [...block.children].forEach((row, index) => {
    ...
    if (index === 2) {
      ...
      if (isExternalLinkFlag === 'true') {
        ...
        block.setAttribute('data-link-origin', 'internal');
      }
    }
  });
  function isInternalLinkDomFn() {
  ...
  if (block.getAttribute('data-link-origin') === 'internal') {
    isInternalLinkDomFn();
  } else {
    ...
    if (externalUrl && (externalUrl.startsWith('http://') || externalUrl.startsWith('https://'))) {
      ...
      // 遍历子节点，移除除外部视频容器以外的其他节点
      [...block.childNodes].forEach((child) => {
        if (child !== externalVideoBox) {
