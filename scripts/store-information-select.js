...
export function storeInformationSelect() {
  ...
  selectSelected1.textContent = placeholder1?.firstElementChild.textContent?.trim();
  ...
  selectItemTagTitle.className = 'select-tag-title';
  ...
  selectSelected2.textContent = placeholder2?.firstElementChild.textContent?.trim();
  ...
  function renderItemTags(selectedTag) {
    ...
    aList.forEach((a) => {
      ...
      a.querySelectorAll('.store-information-card-wrapper').forEach((b) => {
        // 逗号字符串 → 数组
        const itemTagStr = b.getAttribute('data-item-tag') || '';
        const itemTags = itemTagStr.split(',').map((index) => index.trim()).filter(Boolean);
        itemTags.forEach((tags) => itemSet.add(tags));
      });
    });
  ...
  confirmBtn.addEventListener('click', () => {
    ...
    aList.forEach((a) => {
      ...
      bItems.forEach((b) => {
        const itemTagStr = b.dataset.itemTag || '';
        const itemTags = itemTagStr.split(',').map((i) => i.trim()).filter(Boolean);
        // 只要包含就显示
        const show = !it || itemTags.includes(it);
